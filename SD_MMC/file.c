...
#if (MMC_LS==TRUE)
	static void lsRowsOfClust (fptr_t uputs_ptr, unsigned long start_sec);	// zeigt reihen eines clusters an, ab start_sec, muss zeiger auf eine ausgabe funktion überbeben bekommen
#endif
//*******************************************************************************************************************************
...
//*******************************************************************************************************************************
unsigned char ffopen(char name[], unsigned char rw_flag){
...
void fflushFileData(void){
	...
	unsigned long save_currentSectorNr;
	...
	if( file.length < (file.seek + file.cntOfBytes) )	{
		...
		#if (MMC_ENDIANNESS_LITTLE == TRUE)
			*(unsigned long*)psector = new_length;
		#else
			*psector++ = new_length;
...
// *******************************************************************************************************************************
void ffseek(unsigned long offset){

	unsigned long sectors;							// zum durchgehen durch die sektoren der datei
...
// *******************************************************************************************************************************
static void lsRowsOfClust (fptr_t uputs_ptr,unsigned long start_sec){

	unsigned short row, row_index;				// reihen
	unsigned char sec;				// sektoren
	char tmp[12];			// tmp string zur umwandlung
	...
	do{
		...
		for( row=0; row<16; row++ ){		// geht durch reihen des sektors
			...
			if( (fat.sector[row_index+11]==0x20||fat.sector[row_index+11]==0x10) && (fat.sector[row_index]!=0xE5 && fat.sector[row_index]!=0x00) ){
				
				// namen extrahieren und anzeigen
				for( i=0 ; i<11 ; i++ ){
					tmp[i]=fat.sector[row_index+i];
				}
				tmp[i]='\0';
				uputs_ptr(tmp);
...
void ffmkdir(char *name){
	...
	unsigned short j;								// variable zum zaehlen der sektor bytes auf dem puffer fat.sector.
	unsigned char l;
	char name_long[12] = "           \0";
	for(i=0;i<12;i++){
		if( (name[i] == '\0') || (name[i] == '\n') || (name[i] == '\r') ){ ////TODO: \n und \r entfernen
	...
	if(TRUE==fat_loadFileDataFromDir(name_long))			// prueft ob dirname im dir schon vorhanden, wenn ja, abbruch !
		return ;
	...
	fat_makeFileEntry(name_long,0x10); 				// legt ordner im partent verzeichnis an.
	...
	fat.sector[33] = '.';
		...
		*(unsigned short*)vsector=(file.firstCluster&0xffff0000)>>16;		// hi word	von cluster (20,2)
		vsector += 6;

		*(unsigned short*)vsector=(file.firstCluster&0x0000ffff);			// low word von cluster (26,2)

		vsector += 26;

		*(unsigned short*)vsector=(fat.dir&0xffff0000)>>16;				// hi word	von cluster (20,2)
		vsector+=6;

		*(unsigned short*)vsector=(fat.dir&0x0000ffff);					// low word von cluster (26,2)

	#else
		unsigned char *psector =& fat.sector[20];
...
unsigned char ffrm(char name[] ){
		...
		unsigned long parent;		// vaterverzeichnis cluster
		unsigned long own;			// cluster des eigenen verzeichnis
		unsigned long clustsOfDir;	// um durch die cluster eines dirs zu gehen
	...
	row = file.row; //in file.row steht die Zeile der Datei, weil sie vorher gesucht und gefunden wurde
	row = row<<5;
	...
	fat.sector[row] = 0xE5;
		...
		//// ist ordner. jetzt rekursiv durch alle zweige/aeste des baums, den dieser ordner evtl. aufspannt, und alles loeschen!
			...
			do{
				...
				}while( !((clustsOfDir==0xffff && fat.fatType==16)||(clustsOfDir>=0x0fffffff && fat.fatType==32)) );		// geht durch cluster des dirs.
...
// *******************************************************************************************************************************
void ffwriten( unsigned char *s, unsigned short n ){
	unsigned short i = 0;
    do{
    	ffwrite(*s++);
    }while( ++i < n );
