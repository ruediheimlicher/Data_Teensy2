...
static unsigned char 	fat_loadFileDataFromCluster(unsigned long sec ,char name []);
#if (MMC_WRITE==TRUE)
	static void 			fat_getFreeRowsOfDir(unsigned long dir,unsigned char row_cnt);
	static unsigned char 	fat_getFreeRowsOfCluster(unsigned long secStart, unsigned char rows);
	static void 			fat_addClusterToDir(unsigned long lastClusterOfDir);
	#if (MMC_LFN_SUPPORT==TRUE)
		static void 			fat_makeLfnDataEntrys(unsigned char name[],unsigned char row_count);
		static unsigned char 	fat_lfn_checksum(unsigned char name[]);
	#endif
#endif
#if (MMC_TIME_STAMP==TRUE)
	static  unsigned short 	fat_getTime(void); // infos, siehe config.h in sektion: SCHALTER und ZEIT FUNKTIONEN
	static  unsigned short 	fat_getDate(void);
#endif
//***************************************************************************************************************
...
//***************************************************************************************************************
unsigned char fat_writeSector(unsigned long sec){
...
//***************************************************************************************************************
unsigned char fat_loadSector(unsigned long sec){
...
// ***************************************************************************************************************
unsigned long fat_clustToSec(unsigned long clust){
...
// ***************************************************************************************************************
unsigned long fat_secToClust(unsigned long sec){
...
void fat_loadRowOfSector(unsigned short row){
		...
		file.length=*(unsigned long*)vsector;		// 4 byte von file.length (28,4)
	#else
...
static unsigned char fat_loadFileDataFromCluster(unsigned long sec ,char name []){
    ...
    unsigned short row, row_index;    // um durch zeilen zu gehen, eigentlich erstes byte einer reihe...
    ...
    do{                      // reihen des sektors pruefen
	  ...
	  row_index = row<<5; //row*32 for performance

	  // 1.) nach einem eintrag mit 0x00 kommt nix mehr => restlicher cluster leer!
      if( fat.sector[row_index]==0x00 ){
        return FALSE;
      }
      // 2.1) ist eintrag ein lfn eintrag?   siehe: http://en.wikipedia.org/wiki/File_Allocation_Table#Long_file_names
      if( (fat.sector[row_index]&0x40) == 0x40 && fat.sector[row_index+11] == 0x0F && fat.sector[row_index] != 0xE5 ){                                      // ist lfn eintrag, jetzt pruefen...
        lfn_state = start;
      }
      // 2.2) ist eintrag ein sfn eintrag? 
      if( (fat.sector[row_index+11] == 0x10 || fat.sector[row_index+11] == 0x20) && fat.sector[row_index] != 0xE5 && lfn_state != readout){
        // vergleich von sfn dateinamen
        i = 0; j = 0;
        do{          
          if( fat.sector[row_index+i] == 0x20 ){     // ueberspringen von leerzeichen auf der karte
            i++; continue;
          }
          ...
          if( fat.sector[row_index+i] != toupper(name[j]) )  break;
          i++; j++;
        }while(i<11);
        // datei gefunden
        if( i == 11 && j != 11){
          lfn_state = readout;                                      // ist sfn eintrag jetzt auslesen
          checksum = fat_lfn_checksum( &fat.sector[row_index] );
        }
      } 
      ...
      if( lfn_state == start || match !=0 ){
        i=12;
        do{
          if( fat.sector[row_index+map[i]]!=0x00 && fat.sector[row_index+map[i]]!=0xFF ){  // gueltiger buchstabe. ist gueltig wenn, es nicht 0 oder ff ist.
            if( fat.sector[row_index+map[i]] == name[name_length-match] ){    // vergleich mit original, ist treffer?
        ...
        if( (name_length-match) == -1 && (fat.sector[row_index]&0x01) == 0x01){
          ...
          checksum = fat.sector[row_index+13];  // checksumme sichern zum vergleich mit der errechneten anhand des kurzen dateinamens. in naechstem zustand vergleich.
        }
      }
      // 4.) gesuchte reihe auf stuct file laden zum weiter verarbeiten. das ist der sfn eintrag, mit dem firstCluster, der laenge usw...
      if( lfn_state == readout && fat.sector[row_index+11] != 0x0F ){
        lfn_state = wait;
        fat_loadRowOfSector(row);
        file.row = row_index>>5;                  // reihe sichern, ist fuer ffrm wichtig
        if(checksum==fat_lfn_checksum(&fat.sector[row_index])){  // pruefen ob checksumme stimmt...wenn ja alles klar :)
          file.entrySector = file.currentSectorNr;    // sichern des sektors in dem der sfn dateieintrag steht.
          return TRUE;
        }
      }

      }while( (row++) < 16 );      // mit row+32 springt man immer an die 1. stelle einer reihe (zeile) und das durch einen ganzen sektor
...
static unsigned char fat_loadFileDataFromCluster(unsigned long sec ,char name []){
  ...
  unsigned short row, row_index;		// um durch zeilen nummern zu zaehlen
  unsigned char sectors;		// um durch sektoren zu zaehlen
  unsigned char i;			// zaehler fuer datei namen vergleich
  unsigned char j;
  ...
  do{										// sektoren des clusters pruefen
	...
	do{										// reihen des sektors pruefen
		...
		if( fat.sector[row_index] == 0 ){		// wenn man auf erste 0 stoesst muesste der rest auch leer sein!
			return FALSE;
			}
		// normaler eintrag, ordner oder datei.
		if( (fat.sector[row_index+11] == 0x10 || fat.sector[row_index+11] == 0x20) && fat.sector[row_index] != 0xE5 ){
			// vergleich von sfn dateinamen
			i = 0; j = 0;
			do{					
				if( fat.sector[row_index+i] == 0x20 ){ 		// ueberspringen von leerzeichen auf der karte
			...
			// datei gefunden
			if( (i == 11) && ((name[j] == 0) ||(name[j] == 0x2e)) ){
				file.row = row_index>>5;								// zeile sichern.
				fat_loadRowOfSector(row);						// datei infos auf struct laden
		...
		}while( (row++) < 16 );			// springt immer auf zeilenanfang eines 32 byte eintrags im sektor
	}while(++sectors<fat.secPerClust);		// geht durch sektoren des clusters
...
unsigned char fat_loadFileDataFromDir(char name []){
	...
	unsigned char	 sectors;	// variable um durch root-dir sektoren zu zaehlen bei fat16
	unsigned long 	clusters;	// variable um durch cluster des verzeichnisses zu gehen
...
void fat_makeFileEntry(char name [],unsigned char attrib){
	...
	unsigned char sectors;				// um bei fat16 root dir sektoren zu zaehlen
	unsigned long dir;				// zum bestimmen ob im root-dir oder woanders

	#if (MMC_LFN_SUPPORT==TRUE)
		unsigned char need_rows;		// reihen die benoetigt werden um lfn und sfn zu speichern
		unsigned char name_length; 	// laenge des langen dateinamen
...
void fat_makeSfnDataEntry(char name [],unsigned char attrib,unsigned long cluster,unsigned long length){
	...
	unsigned char i,j; 		// byte zaehler in reihe von sektor (32byte eintrag)
	...
	do{
		...
		*(unsigned char*)vsector = 0x20;

		if( i < 8 && name[j] != '.'){
			*(unsigned char*)vsector = toupper(name[j]);
			j++;
		}
	...
	vsector++;
		...
		*(unsigned short*)vsector=0x0000;
		...
		if(new==TRUE){
			...
			*(unsigned long*)vsector=date;
			*(unsigned long*)vsector=*(unsigned long*)vsector<<16;
			*(unsigned long*)vsector|=time;
		}
		vsector+=4;
...
void fat_makeSfnDataEntry(char name [],unsigned char attrib,unsigned long cluster,unsigned long length){
	...
	unsigned char i; 			// byte zaehler in reihe von sektor (32byte eintrag)
	unsigned char j;
	unsigned short row;
...
//***************************************************************************************************************
static void fat_makeLfnDataEntrys(unsigned char name[],unsigned char rows){

	unsigned char cnt;				// sequenz nummer des lfn eintrags
	unsigned char c;				// wird gebraucht um unnoetige felder zu bearbeiten
	unsigned char chkSum;			// sfn checksumme
	unsigned short offset;			// reihen offset im sektor
	unsigned char i;				// laufindex fuer name
	unsigned char j;				// laufindex fuer eine zeile
	const unsigned char map[13]={1,3,5,7,9,14,16,18,20,22,24,28,30};	// index von map ist index des zu pruefenden bytes und inhalt ist index des zu pruefenden bytes in der reihe
	unsigned char name_length = strlen((char *)name);					// es wird die laenge des dateinamen zum vergleichen benoetigt!
...
static unsigned char fat_getFreeRowsOfCluster(unsigned long secStart, unsigned char rows){
	...
	unsigned char sectors;			// variable zum zaehlen der sektoren eines clusters.
	unsigned short row;			// offset auf reihen anfang
...
// ****************************************************************************************************************
static void fat_getFreeRowsOfDir(unsigned long dir, unsigned char rows){
...
//***************************************************************************************************************
unsigned long fat_getNextCluster(unsigned long oneCluster){
	...
	if(fat.fatType==16){
		...
		#if (MMC_ENDIANNESS_LITTLE==TRUE)
			return *(unsigned short*)bytesOfSec;
		#else
			bytesOfSec += 1;
			sector = *bytesOfSec--;
			sector <<= 8;
	...
	// FAT 32
		...
		#if (MMC_ENDIANNESS_LITTLE==TRUE)
			return *(unsigned long*)bytesOfSec;
		#else
			bytesOfSec += 3;
			sector = *bytesOfSec--;
			sector <<= 8;
...
//***************************************************************************************************************
void fat_getFatChainClustersInRow( unsigned long offsetCluster){
...
//***************************************************************************************************************
void fat_getFreeClustersInRow(unsigned long offsetCluster){
...
//***************************************************************************************************************
void fat_setClusterChain(unsigned long startCluster, unsigned long endCluster){
...
//***************************************************************************************************************
void fat_setCluster( unsigned long cluster, unsigned long content){
	...
	if(fat.fatType==16){
		...
		#if (MMC_ENDIANNESS_LITTLE==TRUE)
			*(unsigned short*)bytesOfSec = content;
		#else
			*bytesOfSec++ = content;
	...
	// FAT 32
		...
		#if (MMC_ENDIANNESS_LITTLE==TRUE)
			*(unsigned long*)bytesOfSec = content;
		#else
			*bytesOfSec++ = content;
			*bytesOfSec++ = content >> 8;
...
//***************************************************************************************************************
void fat_delClusterChain(unsigned long startCluster){

  unsigned long nextCluster = startCluster;		// tmp variable, wegen verketteter cluster..
...
unsigned char fat_loadFatData(void){
										// offset,size
	unsigned short  	rootEntCnt;		// 17,2				groesse die eine fat belegt
	unsigned short  	fatSz16;		// 22,2				sectors occupied by one fat16
	unsigned long 	fatSz32;		// 36,4				sectors occupied by one fat32
	unsigned long 	secOfFirstPartition;				// ist 1. sektor der 1. partition aus dem MBR
	#if (MMC_ENDIANNESS_LITTLE==TRUE)
		void *vsector;
	#endif
	...
	if(TRUE==mmc_read_sector(0,fat.sector)){				//startsektor bestimmen
		...
		if( fat.sector[457] == 0 ){		
				...
				secOfFirstPartition = *(unsigned long*)vsector;
			#else
		...
		if(fat.rootDir==0){									// FAT32 spezifisch (die pruefung so, ist nicht spezifikation konform !).
				...
				fatSz32=*(unsigned long *)vsector;

				vsector=&fat.sector[44];
				fat.rootDir=*(unsigned long *)vsector;
			#else
...
// *****************************************************************************************************************
static unsigned short fat_getTime(void){
	// rueckgabe wert in folgendem format:
...
// *****************************************************************************************************************
long long fat_getFreeBytes(void){
	long long bytes;							// die tatsaechliche anzahl an bytes
	unsigned long count;								// zaehler fuer cluster in der fat
	unsigned long fatSz;								// zuerst groeße der fat in sektoren, dann letzter sektor der fat in clustern
